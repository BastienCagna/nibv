...
class SplitBrainGeneralInputSpec(CommandLineInputSpec):
...
class SplitBrainGeneralOutputSpec(TraitedSpec):
    split_mask = traits.File(
        desc='Split Brain Mask, Aims writable volume formats')


class SplitBrainGeneral(BVCommand):
    input_spec = SplitBrainGeneralInputSpec
    output_spec = SplitBrainGeneralOutputSpec
    _cmd = 'SplitBrainGeneral'
