...
class NormalizationT1Spm12ReinitInputSpec(CommandLineInputSpec):
...
class NormalizationT1Spm12ReinitOutputSpec(TraitedSpec):
    transformations_informations = traits.File(
        desc='SPM2 normalization matrix,  Matlab file')
    normalized_anatomy_data = traits.File(
        desc='Raw T1 MRI, NIFTI-1 image, SPM image, {normalization: SPM}')
...
class NormalizationT1Spm12Reinit(BVCommand):
    input_spec = NormalizationT1Spm12ReinitInputSpec
    output_spec = NormalizationT1Spm12ReinitOutputSpec
    _cmd = 'NormalizationT1Spm12Reinit'
