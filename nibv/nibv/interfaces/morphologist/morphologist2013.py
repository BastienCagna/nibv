...
class Morphologist2013InputSpec(CommandLineInputSpec):
    t1mri = traits.File(
        exists=True,
...
class Morphologist2013OutputSpec(TraitedSpec):
    t1mri_nobias = traits.File(
        desc='T1 MRI Bias Corrected, Aims writable volume formats')
    histo_analysis = traits.File(
        desc='Histo Analysis,  Histo Analysis')
...
class Morphologist2013(BVCommand):
    input_spec = Morphologist2013InputSpec
    output_spec = Morphologist2013OutputSpec
    _cmd = 'Morphologist2013'
